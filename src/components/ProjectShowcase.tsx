  ...
  return (
    <section id="projects" className="my-4 sm:my-6 max-w-5xl mx-auto px-4 sm:px-6">
      ...
      <div className="grid gap-4 sm:gap-6 mt-4 sm:mt-5 sm:grid-cols-2">
        <ScrollAnimatedProjectCard type="current" />
        <ScrollAnimatedProjectCard type="future" onCollaborationClick={handleCollaborationClick} />
      </div>
    </section>
  );
}

function ScrollAnimatedProjectCard({ type, onCollaborationClick }: { type: 'current' | 'future'; onCollaborationClick?: () => void }) {
  ...
  if (type === 'current') {
    return (
      <div 
        ref={elementRef}
        className="group rounded-xl border border-border bg-gradient-to-br from-orange-50 via-white to-orange-50/30 overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-[1.02] relative min-h-[140px] sm:min-h-[150px] flex items-center justify-center"
        style={{
          transform,
  ...
  return (
    <div 
      ref={elementRef}
      className="rounded-xl border-2 border-dashed border-indigo-300 bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center min-h-[140px] sm:min-h-[150px] hover:bg-gradient-to-br hover:from-indigo-100 hover:to-purple-100 transition-all duration-300 group"
      style={{
        transform,
        transition: 'transform 0.6s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.6s ease-out',
      }}
